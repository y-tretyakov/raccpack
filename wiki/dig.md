...
Код `2` — это сигнал политики, а не ошибка выполнения. Он срабатывает и в JSON-режиме (для CI проверяйте код выхода); при `--json` дополнительное сообщение в stderr не печатается.
