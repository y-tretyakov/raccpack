...
> → полный список поддерживаемого по имени файла: [Что поддерживается](/ru/supported)
...
> → полный список поддерживаемого по содержимому: [Что поддерживается](/ru/supported)
...
> → полный список маркеров и фреймворков: [Что поддерживается](/ru/supported)
...
> → список стратегий: [Что поддерживается](/ru/supported) · настройка: [Конфигурация](/ru/configuration) · команда: [Rinse](/ru/rinse)
...
::: tip
`racc raid --root <dir>` запускает полный цикл сразу для всех найденных проектов под `<dir>` — не нужно вызывать `raid` для каждого проекта по отдельности.
:::
...
- [Концепции архитектуры](/ru/architecture) — как устроен raccpack изнутри.
- [Facade API](/ru/facade-api) — публичный контракт ядра.
- [Дорожная карта](/ru/roadmap) — что уже есть и что планируется.
