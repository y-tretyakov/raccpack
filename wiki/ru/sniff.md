...
::: warning
**`composite_dag` — экспериментальный режим** (с версии `0.3.6`): запуск дополнительно заполняет рекурсивное `stack_tree` для каждого проекта (композитный DAG); плоская сводка `stack` заполняется в обоих режимах. Семантика может меняться до выхода Detect v2 (`0.4.0`).
:::
...
::: tip Вывод дерева в composite_dag
В режиме `composite_dag` (с версии `0.3.6`) после таблицы проектов для каждого проекта печатается отступное дерево с иерархией стека (box-drawing символы). В режиме `priority_table` вывод не меняется — только плоская таблица.
:::
...
| `stack_tree` | объект/null | Рекурсивное дерево стека проекта; `null` в режиме `priority_table`, заполненный объект в режиме `composite_dag` (см. [концепции](/ru/concepts#stack-tree)) |
