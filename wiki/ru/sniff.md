...
::: warning
**`composite_dag` — экспериментальный режим** (с версии `0.3.4`): запуск дополнительно заполняет рекурсивное `stack_tree` для каждого проекта (композитный DAG); плоская сводка `stack` заполняется в обоих режимах. Семантика может меняться до выхода Detect v2 (`0.4.0`).
:::
