...
::: info
Текущая версия: **0.4.5** — Alpha завершена; Detect v2 завершена (D1 ✅; D2 ✅; D3 ✅; D4 ✅ wiki + E2E + batch raid); **Beta B1.4 TUI raid + progress done (0.4.4)**; **TUI Visual System 2.0 done (0.4.5)**.
...
## Уже доступно (Detect v2 0.4.x)

- [x] Композитные детекторы стека (DAG) для монорепозиториев и гибридных проектов.
- [x] Дерево стека в `sniff` (`--detect-mode=dag`), обратная совместимость плоского `stack`.
- [x] `rinse` по DAG-скоупам — очистка мусора только в релевантных поддеревьях.
- [x] Batch raid: `racc raid --root` обрабатывает все проекты в корне сканирования.

## Планируется (Beta 0.5.x)

- [x] TUI skeleton — Ratatui-бинарник с навигацией, темой, экранами (с 0.4.1).
- [x] TUI-экран sniff — таблица проектов, неблокирующий worker sniff, прогресс, навигация j/k (с 0.4.2).
...
- [x] TUI Visual System 2.0 — тема графит+оранж (DTCG v0.2.0), shell с живыми бейджами в sidebar, Overview-дашборд (KPI + карточки проектов), Projects — карточки по умолчанию (`v` циклирует Cards/Table/Tree), activity-лента на широких терминалах, разбивка модулей (с 0.4.5).
- [ ] TUI — reveal modal.
...
| **Detect v2** | 0.4.x | Композитные детекторы / DAG для монорепозиториев; batch raid `racc raid --root` |
