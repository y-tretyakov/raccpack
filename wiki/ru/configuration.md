...
::: info
`"composite_dag"` **экспериментальный** (с версии `0.3.4`): `racc sniff` дополнительно заполняет рекурсивное `stack_tree` для каждого проекта. Неизвестное значение завершается ошибкой с перечислением валидных.
:::
