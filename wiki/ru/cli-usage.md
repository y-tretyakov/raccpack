...
Перед первым запуском создайте конфигурацию одной командой — [racc init](/ru/init):
...
::: danger
Пример с `--remove-sources` удаляет исходные секреты после успешного stash. Подробности и ограничения: [Stash](/ru/stash).
:::
...
Подробно: [Init](/ru/init)

## В разработке

Следующие команды планируются в ближайших версиях (см. [Дорожную карту](/ru/roadmap)):
...
- JSON-вывод никогда не содержит raw-значений секретов — только маскированные превью и хеши (подробнее: [Dig](/ru/dig)).
