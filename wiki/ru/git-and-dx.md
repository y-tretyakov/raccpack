---
title: Git, init и DX
description: Возможности фазы Alpha A4 — git status у находок dig, racc init, подробные логи --verbose без секретов.
---

# Git, init и DX

**Для пользователей CLI.**
Возможности фазы **A4**: git status в dig, `racc init`, `--verbose`.

Нет на этой странице — нет в текущей версии.
...
| [racc init](/ru/init) | Быстро создать `config.toml` и опционально den |
...
::: tip Безопасность логов
В логи никогда не попадают значения секретов и passphrase — только пути,
счётчики и источник passphrase («env» / «tty» / «stdin»). Проверить можно так:
:::
...
::: warning Осторожно с init --force
`racc init --force` перезаписывает конфиг по указанному пути. В скриптах и CI
изолируйте окружение: `RACCPACK_CONFIG=/tmp/my-racc.toml`.
:::
...
*Соответствует Alpha A4 (a4.1–a4.4). Меняете флаги — обновляйте wiki в том же PR.*
