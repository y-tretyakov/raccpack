...
Если команды нет — см. [Установка](/ru/installation).
...
Если проектов нет — проверьте `scan_root` и глубину сканирования (см. [Конфигурация](/ru/configuration)).
...
Первый запуск — **dry-run** (ничего не удаляет); флаг `--yes` удаляет каталоги артефактов сборки (`target`, `node_modules`, …) по стратегиям из конфигурации. Какие стратегии включены по умолчанию и как подключить `jvm`, `go`, `generic` — см. [Конфигурация](/ru/configuration) и [Rinse](/ru/rinse).
