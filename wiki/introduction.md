...
::: info
Сейчас CLI умеет `sniff`, `dig` и `pack`. Целевое поведение `stash`, `rinse` и `raid` описано в справочнике и появится в CLI по мере разработки — см. [Дорожная карта](/roadmap).
:::
