...
- git-статус в отчёте `dig`;
- `racc init` — стартовая конфигурация;
- `racc den` — обзор хранилища.
...
::: info
Сейчас CLI умеет `sniff`, `dig`, `pack`, `stash`, `rinse` и `raid`; дальше — git-статус в `dig` и `racc init`. См. [Дорожная карта](/roadmap).
:::
