...
Страница описывает, **что умеет raccpack прямо сейчас**: как он находит проекты, какие секреты ищет, какие папки пропускает, как чистит мусор сборки и что исключает из архива. Включая возможности Alpha: `stash` (age) и `rinse` (стратегии очистки).
...
Код **2** бывает **только у `dig`**. У `sniff`, `pack`, `stash` и `rinse` — только `0` (успех) и `1` (ошибка).
...
| `racc init` — стартовая конфигурация | Alpha |
...
| [Использование CLI](/cli-usage) | флаги `sniff` / `dig` / `pack` / `stash` / `rinse` |
