...
По умолчанию raid работает **атомарно**: артефакты пишутся во временный `den/staging/{id}/` и переносятся в den только в фазе commit; если commit не удался — эффект откатывается (в отчёте `rolled_back: true`). Флаг `--fail-fast` включает старый режим: остановиться на первой упавшей фазе, уже записанные артефакты остаются в den. После каждого raid пишется манифест со списком артефактов и статусом фаз — без сырых секретов.
