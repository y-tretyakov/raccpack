...
**Текущая веха:** Alpha ✅ закрыта (`v0.3.0`, A1–A4). Следующая: Detect v2 → `0.4.0` → Beta `0.5.0`.  
**Текущая версия:** сверять `docs/VERSION_ROADMAP.md` и `WORKLOG.md` (bump после каждого этапа).  
**Закрыто:** MVP `0.1.0` (sniff, dig, pack + den layout); Alpha `0.3.0` (stash, rinse, raid, git/DX, CI; MSRV 1.85).  
**Статус этапов:** всегда сверять `WORKLOG.md` + wiki, не дублировать здесь вслепую.
