...
**Версия `0.4.6`** — MVP `0.1.0` закрыт; **Alpha `0.3.0` закрыта** (stash / rinse / raid / git+DX). **Detect v2 `0.4.0` закрыт** (DAG-детекторы, очистка по экосистеме, пакетный raid `racc raid --root`, wiki + E2E). **Beta B1.4: TUI-raid готов** (dry-run preview → passphrase-модалка → пайплайн фаз → честный результат; Atomic по умолчанию). **Beta B1.5: opt-in reveal в TUI** — `v` на finding с content-совпадением открывает шаг подтверждения и показывает это одно значение один раз (эфемерно, zeroize при закрытии; в состоянии приложения никогда не хранится). TUI также получил визуальный полиш (без bump): тема графит+оранж через design-токены и shell с брендом `◈ RACCPACK` + live-бейджами + версией в header. **Трек CLI→TUI (0.4.6): T-02 pack flow готов** — отдельный флоу `pack` (preview → confirm → progress → result) с опциями content-deny `c`, zstd `z`, custom output-name `o`; T-00 split app.rs и T-01 Operations hub (без bump). Далее: T-03 stash flow (0.4.7), затем TUI stash/rinse/init/overview, потом Desktop — к Beta `0.5.0`.
...
| **TUI** | Beta (экраны sniff + dig + raid + pack) | Ratatui-таблица проектов, неблокирующий worker sniff/dig/raid/pack, Findings с маскированными деталями, фильтр риска `f`, content-переключение `c`, raid-flow `R` (preview → passphrase → прогресс фаз → результат); opt-in reveal `v` (confirm → значение показано один раз → zeroize при закрытии); pack flow из Operations (preview → confirm → прогресс → результат; content-deny `c`, zstd `z`, output-name `o`); визуальная тема графит+оранж через design-токены, shell с брендом `◈ RACCPACK` + live-бейджами + версией в header (с 0.4.2, dig с 0.4.3, raid с 0.4.4, reveal с 0.4.5, pack с 0.4.6) |
