...
**Версия `0.4.2`** — MVP `0.1.0` закрыт; **Alpha `0.3.0` закрыта** (stash / rinse / raid / git+DX). **Detect v2 `0.4.0` закрыт** (DAG-детекторы, очистка по экосистеме, пакетный raid `racc raid --root`, wiki + E2E). **Beta B1.2: TUI-экран sniff готов** (неблокирующий worker). Следующий: Beta `0.5.0` (TUI + Desktop).
...
| **TUI** | Beta (экран sniff) | Ratatui-таблица проектов, неблокирующий worker sniff, навигация j/k (с 0.4.2) |
