...
Команда: `racc stash`  
Статус: Alpha (после MVP 0.1.0).

Эта страница описывает **ровно то поведение**, которое реализует `raccpack` в фазе A1. Если флаг или путь не указаны здесь — их нет в текущей версии.
...
3. Если нет ни env, ни TTY — ошибка с подсказкой задать `RACCPACK_PASSPHRASE`.
