...
Если команды нет — см. [Установка](installation.md).
...
> **Note** Пути могут содержать `~` и относительные компоненты — raccpack сам приведёт их к абсолютным.
...
Если проектов нет — проверьте `scan_root` и глубину сканирования (см. [Конфигурация](configuration.md)).
...
> **Note** В выводе никогда не появляются исходные значения — только маскированные превью и уровень риска.
...
На данный момент CLI умеет `sniff` и `dig`. Дальше по дорожной карте — команды упаковки и полного цикла `raid`:

- [Использование CLI](cli-usage.md) — полный справочник команд.
- [Основные понятия](concepts.md) — что такое den, риски и фазы.
