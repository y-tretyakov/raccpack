...
- **Пакировать проекты** — каждый проект упаковывается в отдельный архив `tar.zst` без секретов и мусора и складывается в «den».
...
> **Note** На момент публикации этой документации доступен CLI с командами `sniff` и `dig`. Команды `stash`, `rinse`, `pack` (в CLI) и `raid` находятся в разработке — их целевое поведение описано в справочнике.
