...
> **Note** Сейчас CLI умеет `sniff` и `dig`. Целевое поведение `stash`, `rinse`, `pack` и `raid` описано в справочнике и появится в CLI по мере разработки — см. [Дорожная карта](roadmap.md).

## Что дальше

- [Установка](installation.md) — собрать и проверить `racc`.
- [Быстрый старт](quick-start.md) — первый прогон за пять минут.
- [Основные понятия](concepts.md) — den, секреты, риски, фазы.
